...
void measure_cortical_thickness( vector<polygons_struct> &layers ) {
  ...
  for (int i=0; i < (*origin).n_points; i++) {
...
int main(int argc, char *argv[]) {
  ...
  STRING surface1_filename, surface2_filename, output_filename_base;
  int num_layers;
  int n_objects;
  File_formats format;
  ...
  // the following code should be replaced by real argument handling
  initialize_argument_processing( argc, argv );

  if( !get_string_argument( NULL, &surface1_filename ) || 
      !get_string_argument( NULL, &surface2_filename ) ||
      !get_string_argument( NULL, &output_filename_base ) ||
      !get_int_argument( 1, &num_layers ) ) {
  ...
  measure_cortical_thickness( layers );
