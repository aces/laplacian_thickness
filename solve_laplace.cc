...
void gradient_volume(Volume laplace_volume, int *sizes) {
  ...
  for (v1=1; v1 < sizes[0]-1; v1++) {
	...
	if (get_volume_real_value(laplace_volume, v1, v2, v3, 0, 0) > 0) {
	  value = pow(pow((get_volume_real_value(laplace_volume, v1, v2, v3+1,
						0, 0) - 
			  get_volume_real_value(laplace_volume, v1, v2, v3-1,
						0, 0) / 2), 2) + 
		      pow((get_volume_real_value(laplace_volume, v1, v2+1, v3,
						0, 0) - 
			  get_volume_real_value(laplace_volume, v1, v2-1, v3,
						0, 0) / 2), 2) + 
		      pow((get_volume_real_value(laplace_volume, v1+1, v2, v3,
						0, 0) - 
			  get_volume_real_value(laplace_volume, v1-1, v2, v3,
						0, 0) / 2), 2), 0.5);

	  set_volume_real_value(laplace_volume, v1, v2, v3, 0, 0, value);
	}
      }
    }
  }
